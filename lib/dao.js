...
exports.get_wilddog_strategy = function(){
    if(wilddog_config.count <= 0) {
        ...
        _findAll(query).then(
            function (results) {
                if (results.length > 0) {
                    ...
                    wilddog_config.config = value;
                    wilddog_config.count = 10000;
                    promise.resolve(value);
                } else {
                    promise.reject("query empty");
                }
            },
            function (error_info) {
        ...
        wilddog_config.count -= 1;
        return new AV.Promise(function(resolve, reject){
            if(wilddog_config.config){
                resolve(wilddog_config.config);
            }else{
                reject("get wilddog config error!");
            }
        });
    }
};
    ...
    _findAll(query).then(
        function (results) {
            ...
            results.forEach(function (result) {
                var data = {
                    "senzId": result.id,
                    "perMinScale": result.get("perMinScale"),
...
//    query.ascending("behaviorLastUpdatedAt");
    return query.first().then(
        ...
        function (timestamp){
            return _searchEarliestAfterLastUpdateSenzTime(user_id, timestamp.getTime(), behavior_len)
        },
        function (error){
            return AV.Promise.error(error);
        }
...
var _searchEarliestAfterLastUpdateSenzTime = function (user_id, last_update_time, behavior_len){
    var promise = new AV.Promise();
    var user = AV.Object.createWithoutData("_User", user_id);
    var query = new AV.Query(Senz);
    query.equalTo("user", user);
    ...
    query.first().then(
        function (result){
            if (result == undefined){
                ...
                // If the latest senz was sooner than (last update time + behavior length)
                // It shows that
            ...
            function (result){
                if (result === undefined || result == null){
                    ...
                    job.forEach(function (status){
                        ...
                        var update_time = new Date(result.get(status)["attributes"]["timestamp"]);
                        console.log(update_time)
                        console.log(result.get(status)["attributes"]["timestamp"])
                        latest_status[status]["updatedAt"] = update_time.toLocaleDateString() + " " +update_time.toLocaleTimeString() ;
                    });
                    var update_time_2 = new Date(result.get("timestamp"));
                    latest_status["updatedAt"] = update_time_2.toLocaleDateString() + " " +update_time_2.toLocaleTimeString();
