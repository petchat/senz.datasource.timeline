            ...
            function (result){
                if (result === undefined || result == null){
                    ...
                    job.forEach(function (status){
                        ...
                        latest_status[status]["timestamp"] = new Date(result.get(status)["attributes"]["timestamp"]);
                    });
                    latest_status["timestamp"] = new Date(result.get("timestamp"));
...
exports.getLatestEvents = function (user_id){
    var query = new AV.Query(Behavior);
    var user  = AV.Object.createWithoutData("_User", user_id);
    query.equalTo("user", user);
    ...
    return query.first().then(
        function (result){
            ...
            console.log("The query result is " + JSON.stringify(prediction, null, 4));
            var latest_events = {};
            prediction.forEach(function (behavior){
                ...
                events.forEach(function (event){
                    if (!_.has(latest_events, event)){
                        latest_events[event] = 0;
                    }
                    else {
                        latest_events[event] += Math.exp(p_prob[event]) * Math.exp(b_prob);
                    }
                });
            });
            // Normalization of Events' probabilities.
            var events = _.keys(latest_events);
            var sum    = 0;
            events.forEach(function (event){
                sum += latest_events[event];
            });
            events.forEach(function (event){
                latest_events[event] = latest_events[event]/sum;
            });
            console.log(latest_events);
            return AV.Promise.as(latest_events);
        },
        function (error){
            return AV.Promise.error(error);
        }
