...
exports.middlewareLog2RawSenzes = function (data){
    ...
    logger.debug(config.logEventType.r2r, "Input Param: " + JSON.stringify(data));
    // Send http request
    ...
    req.post(
        ...
        function (err, httpResponse, body){
            ...
            logger.debug(config.logEventType.r2r, "Received content: " + JSON.stringify(body));
...
exports.middlewareRawSenzes2RefinedSenzes = function (behavior, scale, start_time, end_time){
    ...
    logger.debug(config.logEventType.r2s, "Input Param: " + JSON.stringify(data));
    // Send http request
    var promise = new AV.Promise();
    req.post(
        {
        ...
        function (err, httpResponse, body){
            //console.log(err);
            logger.debug(config.logEventType.r2s, "Received content: " + JSON.stringify(body));
...
exports.middlewareProbSenzes2MultiSenzes = function (prob_senz_list, strategy){
    ...
    logger.debug(config.logEventType.p2m, "Input Param: " + JSON.stringify(data));
    // Send http request
    var promise = new AV.Promise();
    req.post(
        {
        ...
        function (err, httpResponse, body) {
            var res = body;
            logger.debug(config.logEventType.p2m, "Received content: " + JSON.stringify(body));
//            console.log(res);
...
exports.middlewareUserBehavior2Event = function (algo_type, model_tag, seq){
    ...
    var _headers = _.extend({"X-request-Id": request_id}, config.request_info["userbehavior_event"]["headers"]);
    console.log("userbehavior to event's headers: " + JSON.stringify(_headers));
    logger.debug(config.logEventType.anl, "Input Param: " + JSON.stringify(data));
    // Send http request
    var promise = new AV.Promise();
    req.post(
        {
        ...
        function (err, httpResponse, body) {
            ...
            logger.debug(config.logEventType.anl, "Received content: " + JSON.stringify(body));
