...
// - halfHourScale
// - perHourScale
exports.calculateTimeZone = function (the_unix_time, time_scale_type){
    ...
    var the_day  = new Date(year, month, date, 0, 0, 0, 0);
...
//    the_day.setHours(0);the_day.setMinutes(0);the_day.setSeconds(0);the_day.setMilliseconds(0);
    ...
    // Set the time zone.
    if (time_scale_type == "perMinScale"){
        return Math.floor(interval_time / (1000*60));
    }
    else if (time_scale_type == 'tenMinScale'){
        return Math.floor(interval_time / (1000*60*10));
    }
    else if (time_scale_type == 'halfHourScale'){
        return Math.floor(interval_time / (1000*60*30));
    }
    else if (time_scale_type == 'perHourScale'){
...
exports.convertDot2Pound = function (obj){
    var new_obj = {};
    for (var item in obj){
        new_obj[item.replace(".", "#")] = obj[item];
    }
    return new_obj;
};
